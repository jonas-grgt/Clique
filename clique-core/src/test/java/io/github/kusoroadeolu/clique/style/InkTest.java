...
class InkTest {
	...
	class Padding {
		...
		class Horizonal {
			...
			@Test
			void padZero_returnsUnpaddedValue() {
				Ink padded = ink.padding(0);
				assertEquals("hello", padded.on("hello"));
			}

			@Test
			void padValue_addsValueNumberOfSpacesOnEachSide() {
				String result = ink.padding(2).on("x");
				assertEquals("  x  ", result);
			}

			@Test
			void padOne_addsOneSpaceOnEachSide() {
				String result = ink.padding(1).on("x");
				assertEquals(" x ", result);
			}

			@Test
			void padWithColor_appliesPaddingAfterStyle() {
				String result = ink.red().padding(2).on("x");
				assertEquals("  " + ColorCode.RED.ansiSequence() + "x" + StyleCode.RESET.ansiSequence() + "  ", result);
			}

			@Test
			void padWithMultipleStyles_appliesPaddingAfterStyles() {
				String result = ink.red().bold().padding(1).on("x");
				assertEquals(" " + ColorCode.RED.ansiSequence() + StyleCode.BOLD.ansiSequence() + "x" + StyleCode.RESET.ansiSequence() + " ", result);
			}

			@Test
			void negativePad_throwsIllegalArgumentException() {
				assertThrows(IllegalArgumentException.class, () -> ink.padding(-1));
			}

			@Test
			void padReturnsNewInstance() {
				Ink original = ink;
				Ink padded = ink.padding(2);
				assertNotSame(original, padded);
			}

			@Test
			void originalAfterPadNotMutated() {
				ink.padding(2);
				assertEquals("hello", ink.on("hello"));
			}

			@Test
			void chainedPad_chainingReturnsNewInstance() {
				Ink padded = ink.red().padding(2);
				Ink padded2 = padded.padding(1);
				assertNotSame(padded, padded2);
			}

			@Test
			void padOverridesPreviousPad() {
				Ink padded = ink.padding(2).padding(1);
				assertEquals(" x ", padded.on("x"));
			}
		}
	}
